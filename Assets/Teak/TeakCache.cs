...
public class TeakCache : IDisposable
{
    ...
    {
#if CACHE_ENABLED
        if(sqlite3_open(Application.persistentDataPath + "/teak.db", out mDBPtr) != SQLITE_OK)
        {
            Debug.Log("Failed to create Teak request cache. Error: " + sqlite3_errmsg(mDBPtr));
            mDBPtr = IntPtr.Zero;
        }
        else
        {
    ...
    public CachedRequest CacheRequest(Teak.ServiceType serviceType, string endpoint, Dictionary<string, object> parameters)
    {
        ...
        lock(this)
        {
            if(sqlite3_prepare_v2(mDBPtr, sql, -1, out sqlStatement, IntPtr.Zero) == SQLITE_OK)
            {
                ...
                {
                    Debug.Log("Failed to write request to Teak cache. Error: " + sqlite3_errmsg(mDBPtr));
                }
            }
            else
            {
        ...
        lock(this)
        {
            if(sqlite3_prepare_v2(mDBPtr, kCacheReadSQL, -1, out sqlStatement, IntPtr.Zero) == SQLITE_OK)
            {
                while(sqlite3_step(sqlStatement) == SQLITE_ROW)
                {
                    ...
                    request.ServiceType = (Teak.ServiceType)sqlite3_column_int(sqlStatement, 1);
            ...
            {
                Debug.Log("Failed to load requests from Teak cache. Error: " + sqlite3_errmsg(mDBPtr));
            }
        }
        sqlite3_finalize(sqlStatement);
#else
    ...
    public class CachedRequest : Teak.Request
    {
        public int Retries
        {
            get;
        ...
        public bool RemoveFromCache()
        {
            ...
            lock(this.Cache)
            {
                if(sqlite3_prepare_v2(this.Cache.mDBPtr, sql, -1, out sqlStatement, IntPtr.Zero) == SQLITE_OK)
                {
                    if(sqlite3_step(sqlStatement) != SQLITE_DONE)
                    {
                        Debug.Log("Failed to remove request from Teak cache. Error: " + sqlite3_errmsg(this.Cache.mDBPtr));
                        ret = false;
                    }
                }
                else
        ...
        public bool AddRetryInCache()
        {
            ...
            lock(Cache)
            {
                if(sqlite3_prepare_v2(this.Cache.mDBPtr, sql, -1, out sqlStatement, IntPtr.Zero) == SQLITE_OK)
                {
                    if(sqlite3_step(sqlStatement) != SQLITE_DONE)
                    {
                        Debug.Log("Failed to add retry to request in Teak cache. Error: " + sqlite3_errmsg(this.Cache.mDBPtr));
                        ret = false;
                    }
                }
                else
